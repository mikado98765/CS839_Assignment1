...
int main(int argc, char *argv[])
{
    ...
    SdfLayerRefPtr layer = SdfLayer::CreateNew("rotatingSquare.usda");
    ...
    for(int i = 0; i <= 1; i++)
    for(int j = 0; j <= 1; j++)
        objVerts.emplace_back((float)i,(float)j,0.0);
    ...
    timeSamples.push_back(0.0);
    double dist_mult = 1/20.0;
    for (int frame = 1; frame <= nFrames; frame++) {
        timeSamples.push_back((double) frame);
    }
    for (int i = 0; i < nFrames; ++i) {
        double sample = timeSamples[i];
        pointsAttribute.Set(usdPoints, sample);
        for (auto& vertex: objVerts) {
            vertex += dist_mult * GfVec3f(cos(i * 6 * 3.14 / 180), sin(i * 6 * 3.14 / 180), 0);
        }
        usdPoints.assign(objVerts.begin(), objVerts.end());
    }
